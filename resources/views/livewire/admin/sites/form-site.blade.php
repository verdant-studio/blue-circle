    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        @foreach ($blocks as $block)
            <div class="overflow-hidden bg-white rounded-md">
                ...
                @can('sites update')
                    <div class="flex flex-col justify-between p-4 group">
                        <ul class="h-full">
                            @foreach ($block->links()->get() as $link)
                                <li class="mb-2 text-tertiary-600 hover:text-tertiary-800"><a href="{{ $link->link }}" target="_blank">{{ $link->name }}</a></li>
                            @endforeach
                        </ul>
                        <div class="hidden mt-3 group-hover:block">
