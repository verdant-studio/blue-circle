            ...
            <table class="min-w-full divide-y divide-slate-300">
                ...
                @foreach ($pages as $page)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @can('pages update')
                                <a class="inline-block text-green-700 hover:text-green-900" href="{{ route('admin.pages.edit', ['id' => $page->id]) }}">
                                    <svg class="w-6 h-6 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                    <span class="sr-only">{{ __('pages.edit') }}</span>
                                </a>
                            @endcan

                            @can('pages delete')
                                <button class="inline-block text-red-600 hover:text-red-900" wire:click="destroy({{$page->id}})">
                                    <svg class="w-6 h-6 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                    <span class="sr-only">{{ __('pages.delete') }}</span>
                                </button>
                            @endcan
                        </td>
                    </tr>
